...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < ARR_SIZE; i++) {
		if (i % 10000 == 0) {
			...
			/* non blocking. */
			ret = threadpool_add_task(pool,fast_task,arr + i,1);
		}
	...
	printf("%d tasks out of %d have been executed, i: %d.\n",count,ARR_SIZE,i);
	printf("%d tasks out of %d did not execute since the pool was overloaded, i: %d.\n",failed_count,ARR_SIZE,i);
